...
const VEHICLE_SUB_TYPES_MGMT: Record<'mobility' | 'logistics', VehicleSubTypeMgmt[]> = {
  mobility: [
    ...
    },
    {
      ...
      color: '#0ea5e9',
      filterFn: (r) =>
        ['cab', 'hatchback', 'sedan', 'suv'].includes(r.vehicleType?.toLowerCase() ?? ''),
    },
    {
      key: 'shuttle',
  ...
  logistics: [
    {
      ...
      color: '#f59e0b',
      filterFn: (r) =>
        ['tata_ace', 'mini_truck', 'mini_cargo'].includes(r.vehicleType?.toLowerCase() ?? ''),
    },
    {
      key: 'medium-goods',
      label: 'Medium Goods',
      ...
      color: '#0f766e',
      filterFn: (r) =>
        ['dcm', 'medium_goods', 'pickup'].includes(r.vehicleType?.toLowerCase() ?? ''),
    },
    {
      key: 'heavy-truck',
      label: 'Heavy Trucks',
      ...
      color: '#dc2626',
      filterFn: (r) =>
        ['lorry', 'heavy_truck', 'trailer'].includes(r.vehicleType?.toLowerCase() ?? ''),
    },
  ],
};
...
import { CustomerApprovalRow, ApprovalStatus } from '../../types';
...
export const CUSTOMER_MANAGEMENT_CONFIG: Record<
  CustomerManagementCategory,
  CustomerManagementTypeConfig
> = {
  customer: {
    ...
    filterFn: (r) =>
      svc(r) === 'customer' ||
      (!svc(r) &&
        !MOBILITY_VEHICLE_TYPES.includes(r.vehicleType?.toLowerCase() ?? '') &&
...
export const useCustomerManagement = (category: CustomerManagementCategory) => {
  ...
  const vehicleSubTypes = isMultiType
    ? VEHICLE_SUB_TYPES_MGMT[category as 'mobility' | 'logistics']
    : null;
  ...
  const pendingRows = categoryRows.filter(
  ...
  const tabLists: CustomerApprovalRow[][] =
    isMultiType && vehicleSubTypes
      ? [
          approvedRows,
          ...vehicleSubTypes.map((st) => approvedRows.filter(st.filterFn)),
          rejectedRows,
          pendingRows,
        ]
  ...
  const tabs =
    isMultiType && vehicleSubTypes
      ? [
          <Tab
            key='approved'
            icon={<cfg.Icon sx={{ fontSize: '1rem' }} />}
            iconPosition='start'
            label='Approved'
          />,
          ...
          <Tab
            key='draft'
            icon={<PendingActionsIcon sx={{ fontSize: '1rem' }} />}
            iconPosition='start'
            label='Draft'
          />,
        ]
  ...
  const expiryNode = (expiryStr: string | null, createdAt: string): React.ReactNode => {
    ...
    return (
      <span
        style={{ color, fontWeight: remaining <= 6 * 3_600_000 ? 700 : 500, fontSize: '0.8rem' }}
      >
        {label}
      </span>
    );
        ...
        if (cat === 'vehicle-rental') {
          ...
          return (
            <Stack spacing={0.15}>
              ...
              {pref && (
                <Typography
                  sx={{ fontSize: '0.71rem', color: '#64748b', textTransform: 'capitalize' }}
                >
                  {pref}
                </Typography>
              )}
    ...
    // ── Vehicle Docs ───────────────────────────────────────────────────────────
    {
      ...
      format: (_v: unknown, row: CustomerApprovalRow) => {
        ...
        const docs = [
          ...
          {
            label: 'Ins',
            number: r.insuranceNumber as string,
            expiry: r.insuranceExpiry as string,
          },
          { label: 'PUC', number: r.pucNumber as string, expiry: r.pucExpiry as string },
          {
            label: 'Fitness',
            number: r.fitnessNumber as string,
            expiry: r.fitnessExpiry as string,
          },
    ...
    // ── Driver Docs ────────────────────────────────────────────────────────────
    {
      ...
      format: (_v: unknown, row: CustomerApprovalRow) => {
        ...
        const docs = [
          ...
          {
            label: ((r.idProofType as string) || 'ID').toUpperCase(),
            number: r.idProofNumber as string,
