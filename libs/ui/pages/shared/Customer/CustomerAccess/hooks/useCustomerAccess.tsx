...
import { CustomerApprovalRow, ApprovalStatus, ApprovalAction } from '../../types';
...
export const useCustomerAccess = (category: CustomerCategory) => {
  ...
  const vehicleSubTypes = isMultiType
    ? VEHICLE_SUB_TYPES[category as 'mobility' | 'logistics']
    : null;
  ...
  const categoryRows = allRows.filter(
    (r) => cfg.filterFn(r) && (r.status === 'pending' || r.status === 'under_review'),
  );
  ...
  const tabLists: CustomerApprovalRow[][] =
    isMultiType && vehicleSubTypes
      ? [
          categoryRows,
          ...vehicleSubTypes.map((st) => categoryRows.filter(st.filterFn)),
          pendingRows,
        ]
  ...
  const handleDirectAction = async (
    ...
    try {
      const newStatus =
        type === 'approve' ? 'approved' : type === 'reject' ? 'rejected' : 'under_review';
      await authAction({
      ...
      notify.success(`${cfg.title} request ${resultLabel} successfully.`);
      fetchData();
    } catch {
  ...
  const columns: Column<CustomerApprovalRow>[] = [
    ...
    },
    {
      ...
      format: (_v: unknown, row: CustomerApprovalRow) => {
        ...
        const meta = STATUS_META[s as ApprovalStatus] ?? STATUS_META['pending'];
        return (
