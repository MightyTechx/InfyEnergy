...
import { useAuth, useDebounce, useLoader } from '@infyenergy/hooks';
...
function getAccessPath(
  ...
  // Map to existing paths - these reference MOBILITY_ACCESS, LOGISTICS_ACCESS which exist
  if (['bike', 'scooter', 'auto', 'auto_rickshaw', 'cab', 'hatchback', 'sedan', 'suv', 'shuttle', 'bus', 'minibus'].includes(v)) {
    return (P as any).MOBILITY_ACCESS ?? (P as any).DASHBOARD;
  }
  if (['tata_ace', 'mini_truck', 'mini_cargo', 'dcm', 'medium_goods', 'pickup', 'lorry', 'heavy_truck', 'trailer'].includes(v)) {
    return (P as any).LOGISTICS_ACCESS ?? (P as any).DASHBOARD;
  }
  if (MOBILITY_VT.includes(v) || cat === 'mobility') return (P as any).MOBILITY_ACCESS ?? (P as any).DASHBOARD;
  if (LOGISTICS_VT.includes(v) || cat === 'logistics') return (P as any).LOGISTICS_ACCESS ?? (P as any).DASHBOARD;
  ...
  const key = MAP[cat];
  return key ? (P[key] as string) : (P as any).PEOPLE_ACCESS ?? (P as any).DASHBOARD;
}
...
function getCategoryLabel(cat: string, vt?: string): string {
  ...
  const LABELS: Record<string, string> = {
    ...
    parcel: 'Parcel Access',
    'driver-hire': 'Driver Hire Access',
    'vehicle-rental': 'Vehicle Rental Access',
    'mechanic-hire': 'Mechanic Access',
    'petrol-bunk': 'Petrol Bunk Access',
    'ev-charging': 'EV Charging Access',
    showroom: 'Showroom Access',
    'vehicle-finance': 'Vehicle Finance Access',
    'finance-broker': 'Finance Broker Access',
    'insurance-partner': 'Insurance Access',
    user: 'User Access',
  };
...
function getCategoryColor(cat: string, vt?: string): string {
  ...
  const COLORS: Record<string, string> = {
    ...
    parcel: '#ea580c',
    'driver-hire': '#f59e0b',
    'vehicle-rental': '#7c3aed',
    'mechanic-hire': '#78350f',
    'petrol-bunk': '#dc2626',
    'ev-charging': '#059669',
    showroom: '#1d4ed8',
    'vehicle-finance': '#9333ea',
    'finance-broker': '#0f766e',
    'insurance-partner': '#166534',
    user: '#be185d',
  };
...
export const useSharedHeader = () => {
  ...
  const handleProfile = () => {
    handleSettingsClose();
    navigate(consultantMode ? AdminPath.PROFILE : AdminPath.PROFILE);
  };

  const handleAddNew = () =>
    navigate(consultantMode ? AdminPath.CREATE_TICKET : AdminPath.CREATE_TICKET);
  ...
  const handleConsultantPage = () => {
    ...
    setTimeout(() => {
      hideLoader();
      enterConsultantMode();
      navigate(AdminPath.MOBILITY_ACCESS);
    }, 1200);
  };
